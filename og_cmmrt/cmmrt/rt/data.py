...
def load_alvadesc_descriptors(download_directory="rt_data", n=None, split_as_np=True):
    ...
    """
    filename = "organic_descriptors.pklz"
    filename = os.path.join(download_directory, filename)
    url = "https://drive.google.com/uc?id=1g4bT1vkLTjU2VLP0LrS9wdRWB4_71eW-&export=download"
...
def load_alvadesc_fingerprints(download_directory="rt_data", n=None, split_as_np=True):
    ...
    """
    filename = "organic_fingerprints.pklz"
    filename = os.path.join(download_directory, filename)
    url = "https://drive.google.com/uc?id=1kvCyJNo4VmzH-LfjqlQDcZcE38eSN30L&export=download"
    return _load_pickled_data(filename, url, n, split_as_np)
...
def load_cmm_fingerprints(download_directory="rt_data"):
    """Load Alvadesc fingerprints of all organic molecules included in Ceu Mass Mediator (CMM)"""
    filename = os.path.join(download_directory, "SMRT_fingerprints_filtered.csv")
    if not os.path.exists(filename):
        if not os.path.exists(os.path.dirname(filename)):
            os.mkdir(os.path.dirname(filename))
        import gdown
        gdown.download("https://drive.google.com/uc?id=1SRXW4WwntgPdbjpfU6qMzrRmYRsnlPKA&export=download",filename)
...
if __name__ == "__main__":
    a = pd.read_csv("rt_data/SMRT_fingerprints_filtered.csv")
